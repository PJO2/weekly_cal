(function ($) {
   ...
   *      6: []
   *    }
   * No more: returns now a kind of hash like :
   *     day: hour:minute  =>  On|Off
   */
  DayScheduleSelector.prototype.serialize = function () {
    ...
    $.each(this.options.days, function (_, v) {
      ...
      plugin.$el.find(".time-slot[data-day='" + v + "']").each(function () {
        ...
        if (!!end) { selections[v].push([start +"-" + end]); 
                     planning += "   " + v + ":   " + start + "  %3D>   On\n" +
                                 "   " + v + ":   " + end   + "  %3D>   Off\n"; 
                     start = end = false; }
      });
    })
...
console.log (schedule);
   ...
   */
  function hmmAmPm___Orig(date) {
    var hours = date.getHours()
      , minutes = date.getMinutes()
      , ampm = hours >= 12 ? 'pm' : 'am';
    return hours + ':' + ('0' + minutes).slice(-2) + ampm;
  }
  /* modif PJO: seulement les heures plaines */
